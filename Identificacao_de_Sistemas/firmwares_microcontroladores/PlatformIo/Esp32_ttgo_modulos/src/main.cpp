...
/* Iniciando uma instáncia do
   gerador de sinais e do controlador PID.
*/
...
PID mypid(0.02, 0.04, 0.4); // Controlador PID
...
void setup()
{
  ...
  /* Configura o sinal PWM usando para
  ...
  if (executar)
  {
    ...
    if (conf_sistema)
    {
      ...
      if (selecionar_onda == 0)
        ...
        sinal_ref = gerar_ref.referencia_onda_dente_serra(
            freq_ref, ampl, offset, Ts);

      /* Sinal de erro calculado, caso seja menor que zero, desliga o Motor. */
      erro = sinal_ref - (theta_saida - 17.5);  // Para controlador PID
    ...
    else
    {
      ...
      //                      0.2, 0.5, 1.0, Ts);
      sinal_entrada_ma = 0.85; //+ gerar_ref.referencia_onda_quadrada(
                               //     0.4, 0.3, Ts);
    ...
    enviar_dados_serial(&sinal_ref, &theta_saida, &erro,
                        &sinal_controle, &sinal_entrada_ma, &t);
    delay(1000 * Ts);

    if (Serial.available() > 0)
    {
      ler_dados_serial(&ampl, &freq_ref, &offset,
                       &selecionar_onda, &conf_sistema, &executar);
    }
    t += Ts;
  }
  else
